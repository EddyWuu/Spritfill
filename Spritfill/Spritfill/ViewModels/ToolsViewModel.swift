...
enum ToolType: CaseIterable {
    ...
    case pencil, eraser, fill, eyedropper, line, rectangle, circle, gradient, dither, select, shift, flip, pan
...
// Select tool options
@Published var selectBrushSize: Int = 1   // 1x1 ... 5x5 lasso brush
@Published var selectFillMode: Bool = false  // when true, flood-fill selects connected area
...
static func isDrawingTool(_ tool: ToolType) -> Bool {
    switch tool {
    case .pencil, .eraser, .fill, .eyedropper, .line, .rectangle, .circle, .gradient, .dither, .select: return true
...
func applyTool(to pixel: inout Color) {
    ...
    case .eraser:
        pixel = .clear
    case .fill, .eyedropper, .shift, .flip, .pan, .select,
